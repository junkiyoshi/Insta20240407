	...
	for (int j = 0; j < 2; j++) {
		...
		for (int k = 0; k < this->param_list.size(); k++) {
			...
			for (int i = 0; i < len; i++) {
				...
				auto noise_deg = ofMap(ofNoise(this->param_list[k].x, (ofGetFrameNum() + i) * 0.006), 0, 1, -360, 360);
				auto noise_radius = ofMap(ofNoise(this->param_list[k].y, (ofGetFrameNum() + i) * 0.0085), 0, 1, -320, 320);
				auto next_noise_deg = ofMap(ofNoise(this->param_list[k].x, (ofGetFrameNum() + i + 1) * 0.006), 0, 1, -360, 360);
			...
			for (int i = 0; i < right.size(); i++) {
				...
				if (j == 0) {
					...
					color.setHsb(ofMap(i, 0, right.size(),  k * 42, k * 42 + 31), 150, 255);
				}
